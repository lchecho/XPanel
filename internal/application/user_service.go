...
func (s *UserService) CreateUser(ctx context.Context, input CreateUserInput) (domain.ID, bool, error) {
	...
	// 兼容结论必须是对**当前**这个 Xray 能力世代做出的：节点重启后配置可能已经变了
	// （例如 policy 被去掉），旧世代的证据不能用来放行新用户（FR-005/FR-029）。
	// 两侧世代都必须已知且相等——世代未知时同样拒绝，宁可让管理员先等一次验证，
	// 也不能在「不知道节点当前是什么能力」的情况下创建用户。
	instance, err := s.store.ManagedInstance(ctx)
	if err != nil {
		return "", false, err
	}
	if instance.CapabilityGeneration <= 0 || template.Template.ValidatedGeneration <= 0 ||
		template.Template.ValidatedGeneration != instance.CapabilityGeneration {
