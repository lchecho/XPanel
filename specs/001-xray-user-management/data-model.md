...
页面状态按优先级派生：`deleted` → `disabled`/`disabling` →
`quota_exceeded`/`disabling` → `enabling` → `active`。同步错误作为附加标记显示，不得覆盖
手动禁用或配额超限的真实业务原因。
...
| `limit_bytes` | integer nullable | null 表示无限；否则必须大于 0 |
...
| `id` | UUID | 主键 |
| `allocation_id` | UUID | FK |
| `type` | enum | `counter_decrease`, `missing`, `reappeared`, `boundary_gap`, `node_restart` |
...
7. RPC 调用永远在事务外；结果用 revision 条件在独立短事务中确认。
