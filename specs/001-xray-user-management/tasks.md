...
- [X] T146 在 `internal/application/traffic_service.go` 合并 20-ID 分批结果时校验每批 `InstanceObservation` 属于同一 boot epoch 且时间顺序一致；若批间 Xray 重启或 observation 不一致，整轮不得提交任何 cursor/total/quota 变更，必须安全重试并记录连续性/健康诊断。增加 21+ allocation 在第一、第二批之间重启的 fake 回归测试，断言无混合 epoch、负增量、重复计量或漏掉的配额封禁 per FR-013 / FR-017 / FR-023 / SC-007 (partial: T137)
- [X] T147 统一 `internal/application/auth_service.go`、`internal/web/handlers/auth.go` 与 SQLite session store 的认证结果语义：失败/限流登录的审计写入错误不得被无条件吞掉；logout 的当前 session 撤销与审计结果必须一致，只有确认撤销后才能记录/返回 succeeded，任一步失败都不得留下“登出成功但 session 仍有效”的审计状态。增加 audit insert、session delete/commit 故障注入测试，并断言 HTTP 不返回成功、session 可用性与 succeeded/failed 审计结果吻合且不泄露用户名或密码 per FR-002 / FR-025 / FR-026 / FR-027 / SC-008 (partial: T140)

---

## Phase 11: Convergence

- [X] T148 CRITICAL 将 `internal/web/handlers/auth.go`、`internal/application/auth_service.go` 与 `internal/persistence/sqlite/sessions.go` 的当前 session 建立/撤销和登录/登出审计收敛为单一、可判定的 SQLite 结果协议，消除 handler 显式 `Commit` 后 SCS middleware 再次提交的窗口，且不得忽略审计失败后的 session 补偿失败；增加“第二次/middleware session commit 失败”“成功登录审计失败且 session delete 同时失败”“logout 审计失败”的故障注入，断言失败响应不留下部分认证状态、每次登录/登出均有与 session 可用性一致的安全审计 per Constitution V / FR-025 / FR-027 / SC-008 / T147 (contradicts)
- [X] T149 阻止 `internal/persistence/sqlite/store_profiles.go` 在任何旧入站漂移身份尚未确认移除时修改 inbound_tag/method/bootstrap：`permanent_failed` drift removal 不能被视为安全终结而放行，或必须为该意图持久化原 inbound tag 并在 profile 改动后继续针对原入站收敛；增加“未知身份移除永久失败 → 尝试改 tag → Xray 恢复”的集成测试，断言旧入站最终无遗留 `xpanel-` 身份 per FR-020 / T144 (partial)
- [X] T150 修正 `internal/adapter/xray/handler.go` 与 `internal/application/traffic_service.go` 的分批实例连续性判定，不得把 Xray `uint32` uptime 推算 boot epoch 的一秒量化抖动误判为重启，同时仍能识别批间真实重启和观察时间倒退；增加相邻批次 boot epoch 相差一秒但 uptime 单调的确定性测试，以及固定 Xray v26.3.27 下 21+ allocation 的真实分批契约测试，断言整轮可提交且真实重启仍整轮丢弃 per FR-013 / FR-017 / FR-023 / T146 (partial)
- [X] T151 补齐漂移移除的外部执行去重：让所有 `NewSynchronizer` 构造路径都具备明确 RPC timeout 并强制 lease 覆盖单次 RPC，在回收过期 `drift_removals` 后先读取原 inbound 的实际身份再决定是否调用 `RemoveUser`；修正 `TestReclaimedDriftRemovalIsNotExecutedTwice` 当前接受两次 `remove_user` 的断言，覆盖 RPC 成功后崩溃与回收重放并断言外部移除、完成状态和审计各至多一次 per FR-021 / T143 (partial)
