...
| II. Xray 集成隔离且版本化 | **CONDITIONAL** | **CONDITIONAL** | 固定 runtime/module，protobuf 只在 `internal/adapter/xray`，不引入 `infra/conf`；但宪章条文未授权面板管理入站监听，需 MINOR 修订，见下 |
...
**处置**：在进入 `/speckit-tasks` 前执行一次 MINOR 宪章修订（1.1.0 → 1.2.0），在原则 II 增补
面板管理入站生命周期的授权与约束。建议条文：
...
|---|---|---|
| 宪章原则 II 需 MINOR 修订以授权面板管理入站生命周期 | 规格的核心价值是每用户独立入站与端口，必须由面板在运行时创建和移除入站；现行条文只授权动态用户与统计读取 | 保持共享入站（即 001 模型）无法提供端口级隔离，与本规格的既定目标冲突；由运维为每个用户手工预配置入站则把面板降级为半自动工具，且无法满足 SC-001 的 5 分钟交付与 SC-013 的端口分配保证 |
